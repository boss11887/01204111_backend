...
router.use(async (req, res, next) => {
    ...
    const testTimeDeta = Math.floor(
        curTest.testTimeInSecond -
            (moment().valueOf() - moment(login.loginTime).valueOf()) / 1000
    )
    const testEndDelta = Math.floor(
        (moment(curTest.endTime).valueOf() - moment().valueOf()) / 1000
    )
    ...
    if (moment().isBefore(curTest.startTime)) {
        ...
        ;(res.locals.std = decoded.std), (res.locals.testId = decoded.testId)

        if (testTimeDeta <= 0) {
            res.status(401).json({ err: 'timeout' })
        } else {
            res.locals.timeLeft =
                testTimeDeta < testEndDelta ? testTimeDeta : testEndDelta
            next()
        }
    }
})
